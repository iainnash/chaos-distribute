...
export const RenderRequest = ({
  ...
  const { data: contractData, error: contractError } = useSWR(
    `https://${CHAINS[chainId].actor}/${transaction.data.to}.json`,
    fetcher
  );
  ...
  const { data: decodeData, error: decodeError } = useSWR(
    contractError || contractData?.guessFromInterface
      ? `https://${CHAINS[chainId].actor}/decode/${transaction.data.to}/${transaction.data.calldata}`
      : undefined,
    fetcher
  );
  ...
  const { data: contractName } = useSWR(
    !contractError
      ? `https://${CHAINS[chainId].actor}/${transaction.data.to}/name`
      : undefined,
    textFetcher
  );
