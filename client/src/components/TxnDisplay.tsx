...
export const TxnDisplay = ({ txn }: { txn: any }) =>
  txn && txn.txn ? (
    <li>
      <strong>{utils.formatEther(txn.txn.value.hex)}</strong>{" "}
      {CHAIN_FE[txn.random.chain]?.currency} was sent to {txn.txn.to}
      {" // "}{" "}
      <a
        className="underline"
        href={`https://${CHAIN_FE[txn.random.chain].scan}/tx/${txn.txn.hash}`}
      >
        view txn
      </a>
